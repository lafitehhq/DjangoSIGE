...
$.Admin.messages = {
    ...
    msgRemove: function(message){
        ...
        $('#modal-msg .modal-body p').text(message);
        $('#modal-msg .modal-title').text('确定吗?');
        $('#modal-msg').modal('show');
...
$.Admin.table = {
    init: function() {
        ...
        dTable = $('#lista-database').DataTable({
            ...
            "language" : {
                "sEmptyTable": "没有数据记录",
                "sInfo": "显示 从第 _START_ 条到第 _END_ 条数据,共 _TOTAL_ 条数据记录",
                "sInfoEmpty": "没有数据记录",
                "sInfoFiltered": "(过滤 _MAX_ 条记录后的结果)",
                "sInfoPostFix": "",
                "sInfoThousands": ".",
                "sLengthMenu": "每页展示 _MENU_ 条结果",
                ...
                "oAria": {
                    "sSortAscending": ": 按升序排序",
                    "sSortDescending": ": 按降序排序"
                },
            }
        });
        ...
        $btnRemove.on('click',function(event){
            ...
            $.Admin.messages.msgRemove("选择的条目将永久地从数据库中删除。");
        ...
        if(typeof data === 'undefined' || !data){
            ...
            for(var i = 0; i < data.length; i++) {
                ...
                if(data[i].model == 'cadastro.cliente'){
                    ...
                    if(ind_ie == '1'){
                        $('#ind_ie_cliente').text('纳税人的培训');
                    }else if(ind_ie == '2'){
                        $('#ind_ie_cliente').text('无登记缴费');
                    }else if(ind_ie == '9'){
