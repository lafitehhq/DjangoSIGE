...
class CompraForm(forms.ModelForm):
    ...
    class Meta:
        ...
        labels = {
            ...
            'fornecedor': _('供货商'),
            'mod_frete': _('运费'),
            'local_dest': _('产品目的地位置'),
            'movimentar_estoque': _('是否备份的吗??'),
            'vendedor': _('推销员'),
            'valor_total': _('汇总金额 (R$)'),
...
class ItensCompraForm(forms.ModelForm):
    ...
    total_com_impostos = forms.DecimalField(widget=forms.TextInput(
        attrs={'class': 'form-control decimal-mask', 'readonly': True}), label='汇总金额', required=False)
    calculo_imposto = forms.CharField(widget=forms.TextInput(
        attrs={'class': 'hidden', 'disabled': True}), label='税费计算器', required=False)
