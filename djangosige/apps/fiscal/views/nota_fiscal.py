    ...
    def post(self, request, *args, **kwargs):
        if len(request.FILES):
            ...
            try:
                ...
                messages.error(
                    request, '试图读取XML文件时发现了以下错误: ' + str(e))
        else:
            messages.error(request, '未选择XML文件。')
...
class ConsultarNotaView(CustomTemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid():
            ...
            if chave and nota:
                messages.error(
                    self.request, '只填写一个字段(请参阅数据库或关键字)。')
                return self.form_invalid(form)
